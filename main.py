...
from utils.tink2sber import tink2sber
...
categories = {
    ...
    19: Category(id=19, name="Household", mcc=[5942, 5921, 99992349, 5999, 5200, 5251, 5331, 5311, 7210]),
    20: Category(id=20, name="Debt", mcc=[]),
    21: Category(id=21, name="Renovation", mcc=[]),
    22: Category(id=22, name="Tax", mcc=[]),
}

year, month = 2025, 3
all_days_of_month = get_all_days_of_month(year, month)
last_day = get_last_day_of_month(year, month)
holidays: list = []  # get_all_days_between(date(year, month, 1), date(year, month, 2))
...
if __name__ == '__main__':
    ...
    for file in os.listdir(f'./{month_name}'):
        if file.endswith('.json'):
            json_ops: list = []
            json_content = json.load(open(f'./{month_name}/{file}'))
            if json_content.get('body'):
                json_ops = json_content['body']['operations']
            elif json_content.get('payload'):
                print('It is seems like T-bank format. Try to parse')
                json_ops = tink2sber(json_content['payload'])
...
# Check if all days of months in operation list
...
for uuid, operation in operations.items():
    if uuid in processed_payments:
        print(f'Payment {uuid} has been processed earlier. Skipped')
        continue
    try:
        if not (operation.get('operationAmount') and operation['operationAmount']['amount']) \
                or (operation.get('form') and operation['form'] == 'UfsTransferSelf') \
                or (operation.get('state') and operation['state']['category'] != 'executed'):
            print(
                f'Operation {uuid} {operation["description"]} for {operation["correspondent"]} do not have write-off. Will be skipped')
